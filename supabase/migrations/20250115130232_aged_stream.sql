...
DO $$ 
BEGIN
  DROP POLICY IF EXISTS "user_roles_select_policy_v4" ON user_roles;
  DROP POLICY IF EXISTS "user_roles_insert_policy_v4" ON user_roles;
  DROP POLICY IF EXISTS "user_roles_update_policy_v4" ON user_roles;
EXCEPTION WHEN undefined_object THEN NULL;
END $$;

-- Create new policies with better permissions
CREATE POLICY "user_roles_select_policy_v5"
  ON user_roles
  FOR SELECT
  USING (true);

CREATE POLICY "user_roles_insert_policy_v5"
  ON user_roles
  FOR INSERT
  WITH CHECK (true);

CREATE POLICY "user_roles_update_policy_v5"
  ON user_roles
  FOR UPDATE
  USING (
    EXISTS (
